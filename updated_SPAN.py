...
class Conv3XC2(nn.Module):
    ...
    def update_params(self):
        with torch.no_grad():
            ...
            b2 = (
                self.conv[1].bias.data.clone()
                if self.conv[1].bias is not None
                else None
            )
            w3 = self.conv[2].weight.data.clone()
            b3 = (
                self.conv[2].bias.data.clone()
            ...
            for g in range(self.groups):
                ...
                weight_concat_g = (
                    F.conv2d(
                        w_g.flip(2, 3).permute(1, 0, 2, 3), w3_g, padding=0, stride=1
                    )
                    .flip(2, 3)
                    .permute(1, 0, 2, 3)
                )
                ...
                if b1 is not None and b2 is not None and b3 is not None:
                    ...
                    bias_concat_g = (w3_g * b_g.reshape(1, -1, 1, 1)).sum(
                        (1, 2, 3)
                    ) + b3[g * group_out_channels : (g + 1) * group_out_channels]
                    bias_concat[
                        g * group_out_channels : (g + 1) * group_out_channels
                    ] = bias_concat_g
...
if __name__ == "__main__":
    ...
    print("Avg time: {:.4f}".format(total_time / 100.0))
